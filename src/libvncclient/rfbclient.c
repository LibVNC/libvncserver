...
extern rfbBool rfbClientEncryptBytes(unsigned char* bytes, char* passwd);
extern rfbBool rfbClientEncryptBytes2(unsigned char *where, const int length, unsigned char *key);
...
HandleVncAuth(rfbClient *client)
{
    ...
    if (client->serverPort!=-1) { /* if not playing a vncrec file */
      ...
      if (!rfbClientEncryptBytes(challenge, passwd)) {
...
HandleUltraMSLogonIIAuth(rfbClient *client)
{
  ...
  if (!rfbClientEncryptBytes2(username, sizeof(username), (unsigned char *)key)) {
    rfbClientLog("Encrypting username failed\n");
    return FALSE;
  }
  if (!rfbClientEncryptBytes2(password, sizeof(password), (unsigned char *)key)) {
